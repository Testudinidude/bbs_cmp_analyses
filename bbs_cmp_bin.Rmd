...
CMP_bbs_pcap_join_pcaponly_landcomm<-left_join(CMP_bbs_pcap_join_pcaponly_landcomm,observers,by="simp_plot")
...
CMP_bbs_pcap_join_pcaponly_detecthistory_observer<-CMP_bbs_pcap_join_pcaponly_landcomm %>% 
...
sitechecks_observed_surveyhistory<-sitechecks_observed %>%
  ...
  summarise(`1`=paste(unique(unlist(strsplit(paste(na.omit(`1`), collapse="; "),";"))),collapse="; "),
            ...
            `8`=paste(unique(unlist(strsplit(paste(na.omit(`8`), collapse="; "),";"))),collapse="; ")) %>%
  replace(.=="",NA)
...
occ_null <- ~1
det_null <- ~1
det_null_randobs<- (1|observer)
det_day <- ~ scale(day) 
det_day_randobs <- ~ scale(day) + (1|observer)
det_daytod<- ~ scale(day)+scale(tod)
det_daytod_randobs <- ~ scale(day)+scale(tod)+ (1|observer)
det_daytodquad <- ~ scale(day)+scale(tod) +I(scale(day)^2)
det_daytodquad_randobs <- ~ scale(day)+scale(tod) +I(scale(day)^2) + (1|observer)
det_season <- ~ scale(seasons)
det_season_randobs <- ~ scale(seasons) + (1|observer)
det_dayyearseason <- ~ scale(dayyear) + scale(seasons) 
det_dayyearseason_randobs <- ~ scale(dayyear) + scale(seasons) + (1|observer)
det_dayyeartodseason <- ~ scale(dayyear)+scale(tod)+scale(seasons)
det_dayyeartodseason_randobs <- ~ scale(dayyear)+scale(tod)+scale(seasons) + (1|observer)
...
ggplot(aes(x=Mean,y=spp_tolorder),data=covars_ldi)+geom_point()+geom_errorbarh(aes(xmin=X5.,xmax=X95.))+theme_classic(base_size=12)+xlab("Covariate estimate")+ylab("Landscape development index")
...
migratecovars<-read.csv("outmslandveg_covar_migratory_sig.csv",header=T)
