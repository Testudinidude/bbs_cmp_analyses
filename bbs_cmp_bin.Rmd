...
You can think of this as a distance-based analog to PCA. 
...
text(bird_pcoa,display="species",col="red",cex=0.6) 
...
We can also use the NMDS to determine species that are most differentiated across site (=plots).
...
plot_nmds <- ggplot(plot_df, aes(x = NMDS1, y = NMDS2, color = reservatio)) +
...
This is a bit hard to interpret, but this table:

```{r, echo = FALSE}
fitnmds_pvals
```

reports the species that contribute most to community-level differences.

We can then select only those species that are most differentiated (by looking back at the fit_spp object from above - so p-values less than or equal to 0.001):

```{r}
fit_spp
```

Some interesting species show up here. 

Summer Migrants: Acadian Fly-Catcher, Baltimore Oriole, Barn Swallow, Blue-Gray Gnatcatcher, Brown Thrasher, Common Yellowthroat, Eastern Kingbird, Eastern Wood-Pewee,  Gray Catbird, Hooded Warbler, House Wren, Indigo Bunting, Louisiana Water-Thrush, Orchard Oriole, Rose-breasted Grosbeak, Red-eyed Vireo, Scarlet Tanager, Tree Swallow, Veery, Warbling Vireo, Willow Flycatcher, Wood Thrush, Yellow Warbler, Yellow-Throated Vireo

Residents:American Robin, Black-Capped Chickadee, Brown-Headed Cowbird, Canada Goose, Cedar Waxwing, Common Grackle, European Starling, Field Sparrow, House Finch, House Sparrow, Mourning Dove, Northern Flicker, Pileated Woodpecker,  Red-winged Blackbird, Song Sparrow, White-breasted Nuthatch

Winter Migrants: Dark-eyed Junco

So 24 summer migrants, 16 residents, and 1 winter migrant. Keep in mind that these are differentiated across *plots*, rather than reservations. We will explore a way to identify reservation-level differentiated species below. 
...
Might be a more useful way to think about differentially-abundant species?
...
rda_reservations_species <- ggplot(plot_df_reservations, aes(x = RDA1, y = RDA2)) +
  coord_fixed() +
  geom_text(data = fit_reservations_spp, aes(label = species))+
  geom_text(data=as.data.frame(fit_reservations),aes(label=reservations))
rda_reservations_species
...
p_beststep_partial<-rda(birdcomm_bin~tol_rel + svp + dicot + cyper + small_tree + Condition(longitude+latitude),data=vegecomm_latlong)

pcap_geog<-varpart(birdcomm_bin,~ tol_rel + svp + dicot + cyper + small_tree,~longitude+latitude,data=vegecomm_latlong)
plot(pcap_geog)

ordiplot(p_beststep_partial)
```

So - most of this variation is associated with these top covariates, and NOT necessarily with latitude and longitude alone.
...
Now, we will produce some exploratory models and plots of basic indicators of vegetative quality, including the Floristic Quality Indicator (FQI) and the Vegetative Index of Biotic Integrity (VIBI).
```{r,warning=FALSE,error=FALSE,message=FALSE}
...
ggplot(aes(x=vibifq,y=S),data=vegecomm_richness)+geom_point()

ggplot(aes(x=fqi,y=S),data=vegecomm_richness)+geom_point()
...
So the number of surveys clearly has an effect on the species richness of the site. This is somewhat difficult to account for, but we will attempt to do so using mixed effects models. 
...
So richness - at a plot level, and after accounting for plot identity - is determined by how many tolerant species are in the associated plot.
...
#### plot-level distances and various indicators

```{r}
birdcomm_bin_bray<-vegdist(birdcomm_bin,method = "bray")

plot_bray_all<-adonis2(birdcomm_bin_bray~.,vegecomm,permutations = 9999)
plot_bray_all
```
...
times<-hm(CMP_bbs_pcap_join_pcaponly$time_start)
time_minutes<-hour(times)*60 + minute(times)
CMP_bbs_pcap_join_pcaponly$time_minutes<-time_minutes
CMP_bbs_pcap_join_pcaponly_detecthistory<-CMP_bbs_pcap_join_pcaponly %>% 
...
CMP_bbs_pcap_join_pcaponly_detecthistory_firstcheck<- CMP_bbs_pcap_join_pcaponly_detecthistory %>% group_by(simp_plot) %>% 
  ...
  unnest(cols = c(date, vibifq, time_minutes, vibifqnotrees, carex, cyper, dicot, 
                  ...
                  SOSP, SWSP, TEWA, TRES, TUTI, TUVU, VEER, VIRA, WAVI, WBNU, 
...
bbs_occobject$y<-array(dim=c(104,164,3))

bbs_occobject$y <- array(c(as.matrix(firstcheck_birds),as.matrix(secondcheck_birds),
                           as.matrix(thirdcheck_birds)), dim=c(104,164,3))
...
bbs_occobject$occ.covs<-as.matrix(vegecomm[,-(1:2)])

bbs_occobject$coords<-as.matrix(vegecomm[,1:2])
...
"RHWO","ROPI","RSHA","RTHA","RTHU","RWBL","SAVS","SCTA","SOSP","SWSP","TEWA",
...
for(i in 1:length(sppnames)){
  plot.dat.ms <- data.frame(x = bbs_occobject$coords[,1], 
                            ...
                            mean.psi = apply(out.ms.global$psi.samples[,i,], 2, mean))
...
#  crs(r)<-"+proj=longlat +datum=NAD83"
  ...
  writeRaster(vr,filename = paste0(sppnames[i],"_occmeanpsi_globalmod_origbbslocs.asc"),overwrite=TRUE)
  ...
  idwrasters[[i]]<-r3
  writeRaster(maxint,filename = paste0(sppnames[i],"_occmeanpsi_globalmod_idw_origbbslocs.asc"),overwrite=TRUE)
  
}

saveRDS(idwrasters,"idwrasters_global.Rdata")

saveRDS(sppnames,"sppnames.Rdata")
