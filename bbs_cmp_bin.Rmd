...
```{r}
CMP_bbs_pcap_join<-read.csv("G:/NaturalResources/Byer/datasets/BBS/CMP_bbs_pcapjoin_elimWC.csv",header=T)
...
birdcomm<-CMP_bbs_pcap_join_pcaponly_pcapsum[,28:(ncol(CMP_bbs_pcap_join_pcaponly_pcapsum))]
...
text(bird_pcoa,display="species",col=color_resid,cex=0.6) 
...
text(bird_nmds,display="species",col=color_resid,cex=0.6) 
...
color_resid_reorder<-color_resid[order(fitconcrda_spp$RDA1)]

ggplot(aes(x=RDA1,y=species),data=fitconcrda_spp_order)+geom_point()+theme_classic(base_size=12)+theme(axis.text.y = element_text(colour = color_resid_reorder))
...
ggord(lda_birdcomm, community, xlim=c(-15,30),ylim = c(-15, 15))
