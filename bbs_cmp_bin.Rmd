...
topmod_predict<-predict(out.ms.fulllandveg,as.matrix(X.0.dat),ignore.RE=TRUE,type="occupancy")

ACFL_ldi3k<-cbind(X.0.dat$ldi_3k,apply(out.ms.fulllandveg$psi.samples[,1,], 2, mean))
colnames(ACFL_ldi3k)<-c("ldi_3k","ACFLocc")



ACFL_pred_ldi3k<-cbind(X.0.dat$ldi_3k,apply(topmod_predict$psi.0.samples[,1,], 2, mean))
colnames(ACFL_pred_ldi3k)<-c("ldi_3k","ACFLocc")

ACFL_ldi3kplot_pred<-ggplot(aes(ldi_3k,ACFLocc),data=data.frame(ACFL_pred_ldi3k))+geom_point()+geom_smooth()

ACFL_ldi3kplot_pred


ACFL_ldi3kplot<-ggplot(aes(ldi_3k,ACFLocc),data=data.frame(ACFL_ldi3k))+geom_point()+geom_smooth(data=data.frame(ACFL_pred_ldi3k))

ACFL_ldi3kplot


HOWA_ldi3k<-cbind(X.0.dat$ldi_3k,apply(out.ms.fulllandveg$psi.samples[,56,], 2, mean))
colnames(HOWA_ldi3k)<-c("ldi_3k","HOWAocc")

HOWA_ldi3kplot<-ggplot(aes(ldi_3k,HOWAocc),data=data.frame(HOWA_ldi3k))+stat_smooth()

HOWA_ldi3kplot


allocc_pred<-cbind(X.0.dat$ldi_3k,X.0.dat$tol_rel,X.0.dat$developed,apply(topmod_predict$psi.0.samples[,,], 3, mean))

colnames(allocc_pred)<-c("ldi_3k","tol_rel","developed","Allocc")

allocc_pred_ldi3kplot<-ggplot(aes(ldi_3k,Allocc),data=data.frame(allocc_pred))+stat_smooth(method="gam")

allocc_pred_ldi3kplot

allocc_tolrelplot<-ggplot(aes(tol_rel,Allocc),data=data.frame(allocc_pred))+stat_smooth(method="gam")
...
allocc_ldi3k<-cbind(X.0.dat$ldi_3k,apply(out.ms.fulllandveg$psi.samples[,,], 3, mean))

colnames(allocc_ldi3k)<-c("ldi_3k","Allocc")

allocc_ldi3kplot<-ggplot(aes(ldi_3k,Allocc),data=data.frame(allocc_ldi3k))+geom_point()
...
allocc_tolrelplot_2<-ggplot(aes(tol_rel,meanpsi),data=data.frame(occu_tolrel_newdata))+geom_line()+geom_ribbon(aes(ymin = meanpsi - sdpsi, ymax = meanpsi + sdpsi),alpha=0.5,fill="grey70")+ylim(c(0,1))+xlim(c(-1,1))
allocc_tolrelplot_2

allocc_tolrelplot_ACFL<-ggplot(aes(tol_rel,meanpsi_ACFL),data=data.frame(occu_tolrel_newdata))+geom_line()+geom_ribbon(aes(ymin = meanpsi_ACFL - sdpsi_ACFL, ymax = meanpsi_ACFL + sdpsi_ACFL),alpha=0.5,fill="grey70")+ylim(c(0,1))
allocc_tolrelplot_ACFL

allocc_tolrelplot_HOWA<-ggplot(aes(tol_rel,meanpsi_HOWA),data=data.frame(occu_tolrel_newdata))+geom_line()+geom_ribbon(aes(ymin = meanpsi_HOWA - sdpsi_HOWA, ymax = meanpsi_HOWA + sdpsi_HOWA),alpha=0.5,fill="grey70")+ylim(c(0,1))
