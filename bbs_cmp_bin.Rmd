...
```{r,warning=FALSE,error=FALSE,message=FALSE}
landcomm<-CMP_bbs_pcap_join_pcaponly_pcapsum[,c(15,16,26,27,149,158,159)]
...
fit_pcoa_spp_order$color_resid[fit_pcoa_spp_order$Residency %in% "NE"]<-"#56B4E9"

fit_pcoa_spp_order$color_resid[fit_pcoa_spp_order$Residency %in% "M"]<-"green3"

fit_pcoa_spp_order$color_resid[fit_pcoa_spp_order$Residency %in% "R"]<-"#E69F00" 
...
ggplot(plot_df_pcoa, aes(x = MDS1, y = MDS1)) +
    coord_fixed(xlim = c(-0.8,0.8),ylim=c(-0.8,0.8)) +geom_segment(data = fit_pcoa_spp_order, aes(x = 0, xend = MDS1, y = 0, yend = MDS1), col = col_pcoa) +geom_text(data = fit_pcoa_spp_order, aes(label = SpeciesCode),colour=col_pcoa)+theme_classic(base_size=24)


ggplot(plot_df_pcoa, aes(x = MDS1, y = MDS1)) +
    coord_fixed(xlim = c(-0.8,0.8),ylim=c(-0.8,0.8)) +geom_point(data = fit_pcoa_spp_order, aes(x =  MDS1, y =  MDS1), col = col_pcoa) +geom_text(data = fit_pcoa_spp_order, aes(label = SpeciesCode),colour=col_pcoa)+theme_classic(base_size=24)

fit_pcoa_spp_order$color_resid<-factor(fit_pcoa_spp_order$color_resid)


ggplot(fit_pcoa_spp_order,aes(x=Residency,y=MDS1,fill=Residency))+stat_summary(geom="col",fill=c("green3","#56B4E9","#E69F00" ))+theme_classic(base_size=24)+scale_y_continuous(expand=c(0,0))#+theme(legend.position="none")
...
allocc_monocotsplot_2<-ggplot(aes(monocots,meanpsi),data=data.frame(occu_monocots_newdata))+geom_line(aes(y=meanpsi_R),color="#E69F00" )+geom_ribbon(aes(ymin = meanpsi_R - sdpsi_R, ymax = meanpsi_R + sdpsi_R),alpha=0.5,fill="#E69F00" )+ylim(c(-0.1,1.1))+xlab("Non-graminoid monocots")+ylab("Community-level Occupancy")+theme_classic(base_size=16)
allocc_monocotsplot_2
